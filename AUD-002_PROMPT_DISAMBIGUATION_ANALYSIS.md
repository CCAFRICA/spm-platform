...
| 3 | flat_percentage | scalar_multiply | rate x base, no conditions |
...
| 6 | linear_function | linear_function | y = slope x input + intercept |
...
| 9 | scalar_multiply | scalar_multiply | rate x base (same as flat_percentage intent) |
...
### PAIR 1: piecewise_linear vs conditional_percentage -> CRITICAL OVERLAP
...
### PAIR 2: tiered_lookup vs piecewise_linear -> MODERATE OVERLAP
...
|-----------|----------------|-----------------|
| Description | "simple rate applied to a base" | "simple rate x base amount, no tiers or conditions" |
| Intent mapping | Both -> scalar_multiply | Same |

**Issue:** These are the same thing. The prompt has two names for one operation. This is not ambiguity (the AI will not produce a wrong result) but redundancy that adds prompt tokens and could confuse the AI about whether to pick flat_percentage or scalar_multiply as the type string.
...
### PAIR 4: conditional_gate vs conditional_percentage -> MODERATE OVERLAP
...
| Key difference | BINARY — one condition, qualify or do not | MULTI-RATE — multiple conditions select different rates |
...
**Impact:** If the AI returns type=conditional_gate but the calculationIntent is a nested chain with rates, the priority inversion (HF-160) will use the intent, which IS a conditional_gate operation. So the executor will handle it. But the semantics may be wrong if the nesting depth does not match.
...
|-----------|----------------|-----------------|
| Description | "y = slope x input + intercept" | "rate x base amount" |
| Key difference | Has an intercept (constant added) | No intercept |

**Overlap risk:** Low. If the plan has a base draw + commission rate, the AI should use linear_function. If there is no base draw, scalar_multiply. The intercept is the differentiator.
...
**Overlap risk:** Low. If the plan has a matrix (attainment x volume -> payout), it is clearly 2D. If it is a single column of tiers, it is 1D. The structural difference is strong.
...
**Fix:** Set temperature: 0 for plan_interpretation and any other classification task where determinism matters. Note: this reduces variability but does not eliminate it (quantization noise). The disambiguation rules are the primary fix; temperature control is defense-in-depth.

### Issue 4: No Prompt Regression Test

When the prompt is modified (HF-160, and now this fix), there is no automated way to verify that existing plans (BCL, Meridian) still interpret correctly. Proposed Standing Rule 43 addresses this but is not implemented.
...
Do NOT include prompt regression testing in HF-162 — that is a separate infrastructure capability (S-NEW-3).
