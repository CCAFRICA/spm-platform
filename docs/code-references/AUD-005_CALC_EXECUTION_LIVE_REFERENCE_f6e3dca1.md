...
**Generated at commit:** `f6e3dca1b412bdeac834b50a593160adcc062e0a` (short: `f6e3dca1`)
**Generated by:** DIAG-035 (refresh of AUD-005 post-HF-206 merge); supersedes `AUD-005_CALC_EXECUTION_LIVE_REFERENCE_5314c365.md`
...
e9b5ab96 HF-206: completion report
eaf5ac5c HF-206: OB-118 derived-metric merge precedence reversal (Shape A)
c889231f Merge pull request #369 from CCAFRICA/diag-034-aud-005-establishment
87511bce DIAG-034: AUD-005 calc-execution live code reference establishment
...
|---|---|---|
| `web/src/app/api/calculation/run/route.ts` | 2260 | HF-206, HF-205, HF-204, HF-202, HF-200, HF-196 P1E |
...
### `web/src/app/api/calculation/run/route.ts:1577-1700` — `perComponentMetrics` population loop (convergence-binding consumption + HF-206 OB-118 merge guard + OB-167 fallback normalization + evaluateComponent + perComponentMetrics.push)
...
### `web/src/app/api/calculation/run/route.ts:1791-1837` — Metrics-handoff site (HF-205 Shape C invariant; intent-executor invocation)
...
### `web/src/app/api/calculation/run/route.ts:1865-1887` — `entityResults.push` site (non-execution `allEntityMetrics` consumers preserved)
...
### `web/src/app/api/calculation/run/route.ts:1391-1422` — VARIANT-DIAG block (OB-190; first-3 dynamic; tenant-agnostic by construction; `eName` resolved at runtime via `entityMap.get(eid)?.display_name`)
...
**Tenant-agnosticism note:** `eName` is resolved per-iteration from `entityMap.get(eid)?.display_name` — runtime data from Supabase `entities.display_name`. The names visible in any trace excerpt reflect whichever tenant ran the calc; for BCL traces, BCL names appear; for Meridian traces, Meridian names appear. No code modification required for cross-tenant operation.

### `web/src/app/api/calculation/run/route.ts:2212` — `addLog COMPLETE` calc-end marker

```typescript
  addLog(`COMPLETE: batch=${batch.id}, entities=${entityResults.length}, total=${grandTotal}`);
```

### `web/src/app/api/calculation/run/route.ts:2214-2260` — handler exit / `NextResponse.json(...)` return
...
**HF-207 patch insertion sites:** `runCalculation:period_complete` and `runCalculation:batch_complete` emissions are inserted between line 2212 (`addLog COMPLETE`) and line 2214 (`return NextResponse.json(...)`). The `[CalcTrace] context` emission referenced in HF-207 §3.3 is at the top of the handler — see §3.1 module-header section.
...
| 2026-05-06 | `f6e3dca1` | DIAG-035 (HF-206 propagation) | HF-206 (PR #370, commit `eaf5ac5c`) modified OB-118 merge precedence at `route.ts:1613-1622` (+5 lines net). All post-OB-118 surfaces shifted +5 (HF-205 Shape C handoff 1786→1791, entityResults.push 1860→1865, addLog COMPLETE 2207→2212). Added VARIANT-DIAG block extract (lines 1391-1422) and handler-exit `NextResponse.json(...)` extract (lines 2214-2260) for HF-207 grounding. Supersedes `5314c365` AUD-005 (now DEPRECATED). |
