...
**Version:** v0.2 LOCKED 2026-06-10 (architect disposition; header amended from stale pre-lock DRAFT line per OB-203 HALT-1 disposition)
**Date:** 2026-06-10
...
**Provenance:** IRA Class A invocation `ecc3824e` (options-based, $1.674465, fired_with_results, 0/41 unit failures; dependency analysis commit 8a44fda). Scope-coherence gate: brief_only invocation (prompt 00d2b85, response c443153, $1.634295, did_not_fire, option_recommendations [], 0/29 unit failures, zero contradicting coherence findings) — passed 2026-06-10. v0.2 incorporates that invocation's three gap findings as DI-9, DI-10, and the synthesis-scope extension of DI-4. Defect evidence: Brasa y Maíz import failure 2026-06-10.
...
- **DI-4 (Failure):** Every comprehension boundary produces a named, durable, user-visible state on failure — including the cross-unit workbook-graph synthesis pass, which is itself a unit in the state machine with a named failure state. A unit without successful comprehension occupies a named state; it cannot appear in a proposal as if comprehended. Silent fallback is structurally impossible at every layer, not procedurally discouraged.
...
- **DI-9 (Algorithm versioning):** Atom fingerprints carry their construction-algorithm version. Any evolution of fingerprint construction (bucketing, pattern flags, distribution features) ships with a bridge so signal accumulated on prior-version fingerprints remains reachable. Accumulated recognition is never stranded by algorithm change. (Scope-coherence GAP-1.)
- **DI-10 (Structural anonymization):** Foundational-scope and vertical-scope atom fingerprints exclude tenant-identifying values by construction — bucketed structural features only, never raw values, identifiers, or names. Anonymization is a property of the fingerprint's structure, not a procedural scrub applied after the fact. (Scope-coherence GAP-3; SR-39 gate hardening.)
...
**Unit state machine.** Every unit (sheet; atoms within it; the workbook-graph synthesis pass as a unit of its own) progresses through named states: `persisted → profiled → recognized(tier) → comprehended → classified → bound`, with `failed_interpretation` and `resolved` as first-class states; the synthesis unit carries the same failure semantics (DI-4 at synthesis scope). `persisted` is state zero and unconditional (DI-1). States are monotonic per unit and independent across units: one sheet's parse failure marks one unit `failed_interpretation` while fifteen proceed.
...
- **G-4 Atom fingerprint algorithm versioning** (scope-coherence GAP-1): resolved by DI-9.
- **G-5 Synthesis-pass failure governance** (scope-coherence GAP-2): resolved by DI-4 at synthesis scope and the §4.2 state machine.
- **G-6 Foundational-scope anonymization enforcement** (scope-coherence GAP-3): resolved by DI-10 — structural by construction, verified at the Korean Test / SR-39 compliance gates (§9).
...
Out of scope: CRP Plans 2+4; tester re-verification arc; any Tier 0 substrate amendment; VG runtime changes. Residuals carried from the IRA arc: v1.1 invocation-reference mirror and CLI-doc staleness corrections in the governance repo; four-table CR produced at this work item's close; scope-coherence gate passed (did_not_fire, zero contradicting findings); four-table CR at work-item close consumes both invocations.

---

*DS-027 — Ingestion Comprehension & Signal Spine Architecture — v0.2 DRAFT — 2026-06-10*
