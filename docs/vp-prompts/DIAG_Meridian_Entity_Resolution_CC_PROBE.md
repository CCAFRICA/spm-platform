# DIAG — Meridian Entity Resolution Defect-Class Verification

**Architect-to-architect content above CC paste block. Rule 29: paste block LAST.**

---

## Architect context (NOT part of CC prompt)

**Sequence:** DIAG (no number — diagnostic, not work-item; architect numbers post-evidence if HF surfaces)
**Predecessor:** HF-196 closed 2026-05-03 SHA `73d52791` (PASS-RECONCILED $312,033 BCL); withdrew HF-197 fingerprint composition drift draft after §7.1 checklist surfaced this is likely Adjacent-Arm Drift at entity-resolution surface, not cache-key indeterminacy
**Live evidence anchor:** Meridian re-import 2026-05-04 02:33 — Tier 1 cache hit + entity resolution `0 created, 67 rows linked across 7 batches`; `Skipping batch ...: identifier column Mes/Cuentas_Nuevas looks like row indices`
**Verification target:** Three §7.1 questions:
1. Q1/Q5 — Adjacent-Arm Drift: does entity resolution consume `field_identities` for HC-primacy gating, or does it run structural heuristics independently?
2. Q2 — Decision-Implementation Gap on Decision 108: does HC primacy operative in SCI classification (HF-196 §1.1.A) propagate to entity-resolution consumer surface?
3. Q3 — Pipeline-Ordering Inversion: is `field_identities` populated at first-import HC-running phase but missing on Tier 1 cache hits?

**This is a value-blind read-only probe.** No DML, no DDL, no fixes. Probe outputs go to architect channel for interpretation. Architect dispositions next-step (HF needed, no HF needed, different defect class) based on probe evidence.

**Capability per memory:** CC executes service-role tsx-script reads + grep + file inspection. Architect interprets. No CC reconciliation against ground truth values.

---

## Pre-paste readiness (architect, before paste)

- [ ] Localhost on `main` HEAD `73d52791` (HF-196 closure)
- [ ] Architect ready to paste tsx-script outputs into architect channel for interpretation
- [ ] No expectation of fix; no expectation of PR; no commits

---

## CC PASTE BLOCK — paste verbatim

```
DIAG: Meridian entity resolution defect-class verification

ROLE: Diagnostic. Read-only. Value-blind. Output evidence; do not draft fixes; do not
propose HFs; do not number anything. Architect interprets.
...
DELIVERABLE: Single output document at docs/CC-artifacts/DIAG_Meridian_Entity_Resolution_Probe.md
with probe results. Commit when complete. Architect reads and dispositions.

═══════════════════════════════════════════════════════════════════════════════════
PROBE PHASE 0 — SCHEMA DISCOVERY (FP-49 guard)
═══════════════════════════════════════════════════════════════════════════════════

Author web/scripts/diag-meridian-er-probe-0-schema.ts (one-shot; do NOT commit; delete
after PASTE):
  ...
  Then: rm web/scripts/diag-meridian-er-probe-0-schema.ts
...
═══════════════════════════════════════════════════════════════════════════════════
PROBE PHASE 1 — CODE ARCHEOLOGY (Q1/Q2 evidence)
═══════════════════════════════════════════════════════════════════════════════════
...
Phase 1.1 — Locate entity resolution code (DS-009 3.3 reference from logs):
  ...
  Identify the canonical entity-resolution function file (likely web/src/lib/supabase/
  entity-service.ts per AUD-001, but verify). Note its absolute path.
...
═══════════════════════════════════════════════════════════════════════════════════
PROBE PHASE 2 — SUBSTRATE STATE (Q3 evidence)
═══════════════════════════════════════════════════════════════════════════════════

Author web/scripts/diag-meridian-er-probe-2-substrate.ts (one-shot; do NOT commit;
delete after PASTE):
  ...
  Phase 2.3 — Import batch state (which imports succeeded; which Tier was hit):
    ...
    ORDER BY created_at DESC
    LIMIT 20;
    (some columns may be in processing_jobs instead — check per Phase 0 / earlier
    schema work)
  ...
  Then: rm web/scripts/diag-meridian-er-probe-2-substrate.ts

═══════════════════════════════════════════════════════════════════════════════════
PROBE PHASE 3 — TRACE LOG REPLAY (Q3 disambiguation)
═══════════════════════════════════════════════════════════════════════════════════

Author web/scripts/diag-meridian-er-probe-3-trace.ts (one-shot; do NOT commit;
delete after PASTE):
  ...
  Then: rm web/scripts/diag-meridian-er-probe-3-trace.ts

═══════════════════════════════════════════════════════════════════════════════════
PROBE OUTPUT — REPORT FORMAT
═══════════════════════════════════════════════════════════════════════════════════

Path: docs/CC-artifacts/DIAG_Meridian_Entity_Resolution_Probe.md
...
COMMIT THE REPORT (only):
  $ git add docs/CC-artifacts/DIAG_Meridian_Entity_Resolution_Probe.md
  $ git commit -m 'DIAG: Meridian entity resolution probe — read-only diagnostic'

DO NOT push. Surface to architect:
  "DIAG probe complete. Report at docs/CC-artifacts/DIAG_Meridian_Entity_Resolution_Probe.md
