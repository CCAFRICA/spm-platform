# HF-297: Plan-Identity Supersession + Post-Commit Assignment Reliability

**Date:** 2026-06-16
**Category:** HF — fixes DIAG-071's two confirmed defects.
**Number:** HF-297 (assigned by architect). VERIFY before committing: `ls docs/vp-prompts/ | grep -iE "^HF-297"` and `ls docs/completion-reports/ | grep -iE "^HF-297"` must both return empty. If either is non-empty, HALT and report — earlier in this arc "HF-297" was used as an unverified label and may have leaked into a branch or commit message; confirm no committed docs artifact already claims 297 before proceeding.
...
Commit `docs/completion-reports/HF-297_COMPLETION_REPORT.md` BEFORE opening the PR. It must contain:
...
*HF-297 · Plan-Identity Supersession + Post-Commit Assignment Reliability · 2026-06-16*
