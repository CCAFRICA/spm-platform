...
| Financial | location_detail, server_detail | **WIRED** | `summary_artifacts_fine` (entity, mesero, date, hour) |
| Financial | staff | **WIRED** | `staff_rollup` tier (40 rows, per location×mesero) — 32× |
| Financial | patterns | **WIRED** | `patterns_rollup`+`patterns_meta` tier (141 rows, per entity×day-of-week) — 41× |
...
`fetchRawDataServer` deleted. Zero RAW. **Zero JS reduces over multi-thousand-row fetches** — staff/patterns now read a ~40/141-row rollup tier, not the 88K fine table. All value-matched to deterministic truth ($100,068,158.15 Sabor / $312,033 BCL). Full evidence + empirical table: `docs/completion-reports/OB-237_COMPLETION_REPORT.md` §ROLLUP + §T-PROOF.
