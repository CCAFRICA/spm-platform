...
# Flags: bare floats used as comparison thresholds — including floats WITH an integer
#   part (e.g. `max <= 1.5`, `1.28 >`), the else-if-chain magnitude-boundary pattern that
#   OB-216 Phase 3″ removed from inferScale. The prior pattern `0?\.[0-9]+` missed those
#   because leading integer digits broke the match; `[0-9]*\.[0-9]+` now catches them.
set -euo pipefail
TARGETS="web/src/lib/intelligence/convergence-service.ts web/src/app/api/calculation/run/route.ts"
# float literals (with optional integer part) used in comparisons, excluding lines marked RATIFIED:
