// HF-217 Phase 5: trigger calculation by direct POST-handler import (middleware
// bypassed by design — script runs in same Node process as the route handler).
// Read calculation_results for entity 007da35a × period 3c2557f4 and paste
// verbatim per directive Phase 5 section.
...
(async () => {
  ...
  for (let i = 0; i < traces.length; i++) {
    ...
    console.log(`modifiers:    ${JSON.stringify(t.modifiers)}`);
    console.log(`finalOutcome: ${JSON.stringify(t.finalOutcome)}`);
    console.log(`componentType: ${JSON.stringify(t.componentType)}`);
    console.log(`componentIndex: ${JSON.stringify(t.componentIndex)}`);
  }
