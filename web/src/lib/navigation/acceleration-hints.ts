...
export function getSmartSuggestions(
  ...
  if (currentPhase === 'import' && (role === 'admin' || role === 'platform')) {
    suggestions.push({
      id: 'suggest-import',
      type: 'action',
      priority: 'high',
  ...
  if (currentPhase === 'calculate' && (role === 'admin' || role === 'platform')) {
    suggestions.push({
  ...
  if (pendingActions > 0 && (role === 'admin' || role === 'platform' || role === 'manager')) {
    suggestions.push({
...
export function getProactiveAlerts(
  ...
  if (criticalItems.length > 0 && (role === 'admin' || role === 'platform')) {
    alerts.push({
  ...
  if (dayOfMonth >= 25 && cyclePhase !== 'closed' && (role === 'admin' || role === 'platform')) {
    alerts.push({
  ...
  if (dataQualityItems.length > 0 && (role === 'admin' || role === 'platform')) {
    alerts.push({
