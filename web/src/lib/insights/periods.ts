...
export async function getCalculatedPeriods(
  tenantId: string,
  // HF-343: optional authenticated scope. Defaults to ALL (admin) so every existing caller is
  // byte-identical; /perform passes the resolved member/manager scope so per-period totals/counts
  // reflect ONLY the visible entity set (a member's own per-period payout, not the tenant's).
