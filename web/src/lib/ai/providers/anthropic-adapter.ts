...
4. For each component declare rateTableCellCount (integer) when the component is backed by a rate table: 1D band table → number of tiers; 2D matrix → rows × columns. Omit the field when the component has no rate table (simple rate × metric, linear function, etc.). TABULAR INPUT (a "=== Sheet: … ===" with a column header row): a rate that comes from a per-row COLUMN — each row carries its OWN value (e.g. a "% AUTORIZADO" column with 0.01, 0.0025, 0.005 …) — is a simple reference × metric, NOT a rate table: OMIT rateTableCellCount (the per-component call will \`reference\` that column, and the engine reads each entity's own value). Declare a rate table ONLY for a FIXED lookup grid of constant cells (tiers / bands / matrix) that is the SAME for every row.
...
EXPRESS THE COMMON COMPUTATIONS IN THE ALGEBRA (illustrative — replace <…> with what the plan states):
  ...
  • A GUARANTEED / CONSTANT PAYMENT (HF-373 D5 — the plan pays a stated amount AS the commission, regardless of any computed result): express the guarantee AS THE PAYMENT ITSELF, never as a rate × base multiplication. A single plan-wide stated amount → constant(<amount>). Per-entity guaranteed amounts carried in a column (each row states its own amount, e.g. a "BASE COMISION" column with 6,000 / 26,000) → reference(<that exact column header>) — the varying-values rule below applies; never hardcode one row's amount. A guarantee that applies only under a condition wraps in a conditional like the eligibility gate.
...
REFERENCE FIELD NAMES — bind to the INPUT, do not invent (CRITICAL):
  • When the input is TABULAR — it shows "=== Sheet: … ===" followed by a HEADER ROW of column names and then data rows — every \`reference\` prime's \`field\` MUST be the EXACT column header name as written in that header row: verbatim, including spaces, accents, %, and original case (e.g. "% AUTORIZADO", "BASE COMISION", "% AUTORIZADO CONFECC"). Do NOT translate, abbreviate, snake_case, or invent a semantic name (NOT "porcentaje_autorizado", NOT "ventas_facturadas_confeccion"). The platform binds each reference to the committed-data column of the SAME name; an invented name has no column to bind to and the component cannot calculate. (A NARRATIVE document — prose / PDF / slides — has no header row; there, name the field from the prose as before.)
