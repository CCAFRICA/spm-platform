...
export function LifecycleCard({
  ...
  const isVialuce = useIsVialuce(); // Vialuce: stepper → indigo/success tokens, current/desc → muted text, action → btn-sec
  ...
  return (
    ...
    >
        ...
        {stages.map((stage, i) => (
          <div key={stage.label} className="flex items-center">
                ...
                <div
                  ...
                  style={
                    stage.status === 'done'
                      ...
                      : stage.status === 'active'
                        ? { background: 'var(--vl-indigo-50)', color: 'var(--vialuce-indigo)', border: '1px solid var(--vialuce-indigo)' }
              ...
              {isVialuce ? (
                <span
                  ...
                  style={{
                    ...
                    color: stage.status === 'done' ? 'var(--vl-success)' : stage.status === 'active' ? 'var(--vl-text)' : 'var(--vl-text-soft)',
                  }}
                >
                  {stage.label}
                </span>
            ...
            {i < stages.length - 1 && (
              isVialuce ? (
                <div
                  className="h-px w-4 sm:w-6 flex-shrink-0 mt-[-16px]"
                  style={{ background: stage.status === 'done' ? 'var(--vl-success)' : 'var(--vl-line)' }}
                />
              ) : (
                <div
                  className={cn(
      ...
      {isVialuce ? (
        <div className="mt-4" style={{ borderTop: '1px solid var(--vl-line-soft)', paddingTop: 12 }}>
          <div className="flex items-center justify-between">
            <div>
              <p style={{ fontSize: '12px', color: 'var(--vl-text-muted)' }}>
                Current: <span style={{ color: 'var(--vl-text)', fontWeight: 'var(--vl-fw-med)', fontFamily: 'var(--vl-font-mono)' }}>{currentState}</span>
              </p>
              <p style={{ fontSize: '11.5px', color: 'var(--vl-text-soft)', marginTop: 2 }}>{LIFECYCLE_DESCRIPTIONS[currentState] || ''}</p>
            </div>
            {nextAction && (
              <button onClick={handleAction} className="btn-sec">
                {nextAction.label}
