...
export function BloodworkCard({
  ...
  const isVialuce = useIsVialuce(); // Vialuce: item rows → light surface w/ danger/gold accent, issue → .pill, action → btn-sec
          ...
          isVialuce ? (
            <div
              ...
              style={{
                ...
                borderLeft: `3px solid ${item.severity === 'critical' ? 'var(--vl-danger)' : 'var(--vl-raw-gold)'}`,
                borderRadius: 'var(--vl-r-sm)',
              }}
            >
              <div className="flex items-start gap-2.5 flex-1 min-w-0">
                {item.severity === 'critical' ? (
                  <AlertCircle className="h-4 w-4 flex-shrink-0 mt-0.5" style={{ color: 'var(--vl-danger)' }} />
                ) : (
                  <AlertTriangle className="h-4 w-4 flex-shrink-0 mt-0.5" style={{ color: 'var(--vl-raw-gold)' }} />
                )}
                <div className="min-w-0">
                  <p className="truncate" style={{ fontSize: '13px', fontWeight: 'var(--vl-fw-med)', color: 'var(--vl-text)' }}>
                    {item.entityName}
                  </p>
                  <div style={{ marginTop: 5 }}>
                    <span className={cn('pill', item.severity === 'critical' ? 'danger' : 'neutral')}>
                      {item.issue}
                    </span>
                  </div>
                </div>
              </div>
              <button onClick={() => onAction?.(item)} className="btn-sec flex-shrink-0">
                {item.actionLabel}
