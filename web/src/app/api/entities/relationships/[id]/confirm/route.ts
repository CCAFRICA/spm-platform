// OB-204 F.3 — POST /api/entities/relationships/[id]/confirm — confirm an inferred edge.
import { NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import { confirmRelationship } from '@/lib/entities/relationships';
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const r = await confirmRelationship(id, sb);
  return r.ok ? NextResponse.json({ ok: true, source: r.source }) : NextResponse.json({ error: 'confirm failed' }, { status: 500 });
}
