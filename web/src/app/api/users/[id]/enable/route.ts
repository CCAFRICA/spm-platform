// OB-204 A.4 — POST /api/users/[id]/enable.
import { NextRequest, NextResponse } from 'next/server';
import { enable, ProvisionError } from '@/lib/auth/provision-user';
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await enable({ targetProfileId: id, actorProfileId: authz.caller.profileId });
    ...
    return NextResponse.json({ error: e instanceof Error ? e.message : 'enable failed' }, { status: 500 });
  }
}
