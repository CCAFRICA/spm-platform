// OB-204 A.4 — POST /api/users/[id]/reset (admin-initiated password recovery link).
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const r = await sendCredentials({ targetProfileId: id, type: 'recovery', actorProfileId: authz.caller.profileId });
