...
export default function CalculatePage() {
  ...
  const handleRunCalculation = async () => {
    ...
    // === CLT-08 DIAGNOSTIC START ===
    const tenantId = currentTenant.id;
    console.log('CLT-DIAG: ========================================');
    console.log('CLT-DIAG: CLT-08 DIAGNOSTIC - TRACE $0 PAYOUTS');
    console.log('CLT-DIAG: ========================================');
    console.log('CLT-DIAG: Tenant ID:', tenantId);
    console.log('CLT-DIAG: Period:', selectedPeriod);

    // Phase 1A: Dump AI Import Context
    const contextKey = `ai_import_context_${tenantId}`;
    const raw = localStorage.getItem(contextKey);
    if (!raw) {
      console.error('CLT-DIAG: NO AI IMPORT CONTEXT FOUND for', tenantId);
      console.log('CLT-DIAG: Available localStorage keys:', Object.keys(localStorage).filter(k => k.includes('import') || k.includes('context')));
    } else {
      const ctx = JSON.parse(raw);
      console.log('CLT-DIAG: === AI IMPORT CONTEXT ===');
      console.log('CLT-DIAG: Top-level keys:', Object.keys(ctx));
      console.log('CLT-DIAG: Roster sheet:', ctx.rosterSheet);
      console.log('CLT-DIAG: Roster employeeId column:', ctx.rosterEmployeeIdColumn);
      console.log('CLT-DIAG: Sheet count:', ctx.sheets?.length);

      if (ctx.sheets) {
        for (const sheet of ctx.sheets) {
          console.log(`CLT-DIAG: Sheet "${sheet.sheetName}": classification="${sheet.classification}", matchedComponent="${sheet.matchedComponent}", fieldMappings=${sheet.fieldMappings?.length}`);
          if (sheet.fieldMappings) {
            for (const fm of sheet.fieldMappings) {
              console.log(`CLT-DIAG:   Field "${fm.sourceColumn}" -> semanticType="${fm.semanticType}" (confidence=${fm.confidence})`);
            }
          }
        }
      }
    ...
    // Phase 2A: Dump aggregated data for first employee
    const aggKey = `data_layer_committed_aggregated_${tenantId}`;
    const aggRaw = localStorage.getItem(aggKey);
    if (!aggRaw) {
      console.error('CLT-DIAG: NO AGGREGATED DATA for key', aggKey);
      console.log('CLT-DIAG: Trying alternate keys...');
      const altKeys = Object.keys(localStorage).filter(k => k.includes('aggregated') || k.includes('employee'));
      console.log('CLT-DIAG: Potential aggregated keys:', altKeys);
    } else {
      const employees = JSON.parse(aggRaw);
      console.log('CLT-DIAG: === AGGREGATED EMPLOYEES ===');
      console.log('CLT-DIAG: Total aggregated employees:', employees.length);

      if (employees.length > 0) {
        const emp = employees[0];
        console.log('CLT-DIAG: === EMPLOYEE 0 FULL RECORD ===');
        console.log('CLT-DIAG: All keys:', Object.keys(emp));
        console.log('CLT-DIAG: Identity:', JSON.stringify({
        ...
        if (cm) {
          console.log('CLT-DIAG: componentMetrics sheets:', Object.keys(cm));
          for (const [sheet, values] of Object.entries(cm)) {
            console.log(`CLT-DIAG: componentMetrics["${sheet}"] =`, JSON.stringify(values));
          }
        } else {
          console.error('CLT-DIAG: NO componentMetrics on employee');
          console.log('CLT-DIAG: Full employee record:', JSON.stringify(emp).substring(0, 2000));
        }
      }
    }

    // Phase 2B: Dump plan components
    const planKeys = Object.keys(localStorage).filter(k => k.includes('plan') && k.includes(tenantId));
    console.log('CLT-DIAG: Plan-related keys:', planKeys);
    const planKey = `clearcomp_plans_${tenantId}`;
    const planRaw = localStorage.getItem(planKey);
    if (!planRaw) {
      console.log('CLT-DIAG: No plan at', planKey, ', trying alternate...');
      const altPlanKey = 'clearcomp_plans';
      const altPlanRaw = localStorage.getItem(altPlanKey);
      if (altPlanRaw) {
        const allPlans = JSON.parse(altPlanRaw);
        const tenantPlans = allPlans.filter((p: { tenantId?: string }) => p.tenantId === tenantId);
        console.log('CLT-DIAG: Found', tenantPlans.length, 'plans for tenant from global store');
      }
    } else {
      const plans = JSON.parse(planRaw);
      console.log('CLT-DIAG: === PLANS ===');
      console.log('CLT-DIAG: Plan count:', Array.isArray(plans) ? plans.length : 1);
    }
    console.log('CLT-DIAG: === END PRE-CALCULATION DIAGNOSTIC ===');
    // === CLT-08 DIAGNOSTIC END ===
